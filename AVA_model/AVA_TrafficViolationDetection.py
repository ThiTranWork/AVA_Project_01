...
#     codec should be compatible to the video format
    codec = cv2.VideoWriter_fourcc(*'XVID')  # for the MP4
#     codec = cv2.VideoWriter_fourcc(*'MPEG')  # for ts file
    out = cv2.VideoWriter(output_path, codec, 30, (width, height)) 
...
Object_tracking(yolo_model, video_input_path, video_output_path, input_size=YOLO_INPUT_SIZE, show=True, iou_threshold=0.1, rectangle_colors=(255,0,0), Track_only = ["person","car","bus"])
